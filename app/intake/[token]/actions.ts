...
export async function submitIntake(token: string, formData: FormData): Promise<Result> {
  ...
  // The intake link is permanent and reusable — Jay can submit many clients
  // from the same link. We only reject a wrong/expired link, never a "used" one.
  ...
  for (const [k, v] of Object.entries(values)) {
    if (CLIENT_COLUMNS.has(k)) insert[k] = v;
  }
